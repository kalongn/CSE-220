    ### reverse_string.asm - reverse the character
    ## string "str"
...
nextCh: 
    lb      $t0,    0($t1)      # get a byte from string
    beqz    $t0,    strEnd      # zero means end of string
    addi    $sp,    $sp,    -4  # adjust stack pointer
    sw      $t0,    0($sp)      # PUSH the t0 register
...
store:  
    ...
    lw      $t0,    0($sp)      # POP a value from the stack
    addi    $sp,    $sp,    4   # end adjust the pointer
    sb      $t0,    0($t1)      # store in string
    addi    $t1,    $t1,    1   # move pointer one character
    j       store
